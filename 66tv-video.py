...
baseurl = "https://www.666dav.com/guochanzipai/163711/index_1_1.html"


#rex1 = re.compile(r'href="(/zipaitoupai.*?)"')
rex1 = re.compile(r'"url":"(https:.*?)"')
...
def getLI(html):
    ...
    allli = soup.find('div', class_='stui-player__video embed-responsive embed-responsive-16by9 clearfix')
    return allli
...
def main():
    ...
    # 请求url地址
    html = getURL(baseurl, head, session)

    # 获取一级页面的所有li
    allli = getLI(html)
    if re.findall(rex1, str(allli)):
        url = re.findall(rex1, str(allli))[0]
        url = url.replace('\\','')
        print(url)

    cmd = 'ffmpeg -i ' + url + ' -c copy ' +'./merge.mpg4'
    os.system(cmd)
    exit()
